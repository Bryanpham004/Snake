...
public class Snake {
	...
	private java.util.List<SnakeBody> snakeSegments = new ArrayList<SnakeBody>();
	...
	public void regenerate() {
		...
		int headX = random.nextInt(GameMain.COLS - length * 2) + length;
		int headY = random.nextInt(GameMain.ROWS - length * 2) + length;
		direction = Snake.Direction
				.values()[random.nextInt(Snake.Direction.values().length)];
		snakeSegments.add(new SnakeBody(headX, headY, length, direction));
	...
	public void shrink() {
		SnakeBody tailSegment = snakeSegments.get(snakeSegments.size()-1);
		tailSegment.shrink();
	...
	public boolean contains(int x, int y) {
		for(int i =0; i< snakeSegments.size(); ++i) {
			SnakeBody segment = snakeSegments.get(i);
